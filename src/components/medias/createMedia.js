...
import { createAgency } from "../../services/agency_api";
...
export default function CreateMediaForm() {
  ...
  const [peak_hour, setPeak_hour] = useState(null);
  const [normal, setNormal] = useState(null);
  const [channel_name, setChannel_name] = useState(null);
  const [production, setProduction] = useState(null);
  const [open, setOpen] = useState(false);

  const handlePeak_hourChange = (event) => {
    setPeak_hour(event.target.value);
  };
  ...
  const handleChannel_nameChange = (event) => {
    setChannel_name(event.target.value);
  };
          ...
          <div className="my-4 flex flex-col gap-4 sm:flex-row">
            <div className="w-full sm:w-1/2">
              <input
                ...
                value={channel_name}
                onChange={handleChannel_nameChange}
              />
            </div>
            <div className="w-full sm:w-1/2">
              <input
                ...
                placeholder="Peak Hour"
                value={peak_hour}
                onChange={handlePeak_hourChange}
              />
            </div>
          </div>
